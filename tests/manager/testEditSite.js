...
manager
  ...
  .then(
    function() {
      console.log('Success')
      return manager.editSite(filename, function(conf) {
        conf
          .find('server')
          .addBlock('location /testarea')
          .addStatement('proxy_pass', 'http://127.0.0.1')
        newConf = conf.toString()
        return conf
      })
    },
    function(err) {
  ...
  .then(function() {
    return manager.parseSite(filename)
  })

  .then(function(conf) {
    assert(conf.toString() == newConf)
    console.log(conf)
  })
